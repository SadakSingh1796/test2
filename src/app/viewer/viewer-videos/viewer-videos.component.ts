...
export class ViewerVideosComponent implements OnInit {
  ...
  constructor(public encyptDecryptService: EncyptDecryptService, private eventsService: EventsService,private videosService: VideosService) { }
  ngOnInit(): void {
  ...
  getEventLiveMatches() {
    ...
    this.videosService.getEventStreamedMatches(this._eventId, this._currentCategoryId, 'streamed').subscribe({
      next: (result: any) => {
        ...
        this._streamedVideos = result.body;
      },
      error: (result: any) => {
        this._showLoader = false;
      },
